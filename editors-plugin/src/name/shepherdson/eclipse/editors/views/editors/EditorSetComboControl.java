...
public class EditorSetComboControl extends ControlContribution
{
    ...
    protected Control createControl(Composite parent)
    {
        ...
        combo.addSelectionListener(new SelectionAdapter()
        {
            ...
            public void widgetSelected(SelectionEvent selectionEvent)
            {
                ...
                {
                    EditorSetSettingsModel selectedSet =
                            settingsService.getEditorSettingsSet(selectedSetName);

                    if (!selectedSetName.equals(Constants.OPEN_EDITORS_SET_NAME))
                    {
                        Map<String, EditorSettingsModel> editors =
                        ...
                        for (IEditor editor : editors.values())
                        {
                            ...
                            if (!selectedSet.getEditorModels().keySet()
                                    .contains(editor.getFilePath()) && editor.isOpened())
                            {
    ...
    private int askToCloseExistingEditors()
    {
        MessageDialog dialog = new MessageDialog(
                ...
                null,
                null,
                "Close open editors or add to selected set?",
                MessageDialog.QUESTION,
                new String[]
                {
                    "Close Editors", "Add Editors"
                },
                0);
